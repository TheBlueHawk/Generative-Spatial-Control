...
def init_attention_func(unet, vae, clip, device, clip_tokenizer):
    ...
    def new_attention(self: "CrossAttention", query, key, value):
        ...
        for prompt_emb, mask_fn in self.mappings:
            ...
            # if mask corresponds to background, then multiply by background intensity
            if mask_fn == make_true_mask:
                hidden_states += background_intensity * torch.matmul(
                    attn_probs, v
                )  # Reshape attn_slice (BH, 1, T)  => (BH, T, 1)
            else:
                hidden_states += (1-background_intensity) * torch.matmul(
                    attn_probs, v
                )
...
def stablediffusion(
    ...
    *,
    ...
    globals()['background_intensity'] = background_intensity
    ...
    with autocast(device):
        ...
        background_emb = get_prompt_emb(background_prompt)
